...
class CaptionTagDataset(Dataset):
    def __init__(self, captions, tag_strs, tokenizer, max_len=128):
        ...
        self.max_tag_len = max(
            len(self.tokenizer.encode(tag_str)) for tag_str in self.tag_strs
        )

        # Preprocess the data
        self._data = [
            {"caption": caption, "tag_str": tag_str}
            for caption, tag_str in zip(self.captions, self.tag_strs)
        ]
        self._preprocess()
    ...
    def __getitem__(self, index):
        ...
        labels = self.tokenizer.encode(
            tag_str, truncation=True, add_special_tokens=False
        )
    ...
    def _preprocess(self):
        def preprocess_item(item):
            ...
            return {"caption": caption, "tag_str": tag_str}
...
def train(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if wandb_project and wandb_run_name:
            wandb.log(
                {"train_loss_average": train_loss, "lr": scheduler.get_last_lr()[0]},
                step=epoch,
            )
...
if __name__ == "__main__":
    ...
    train_dataset = CaptionTagDataset(
        df["caption"][:train_count], df["tag_str"][:train_count], tokenizer
    )
    val_dataset = CaptionTagDataset(
        df["caption"][train_count:], df["tag_str"][train_count:], tokenizer
    )
    ...
    train(
        ...
        wandb_run_name=f"run_{date_str}",
    )
